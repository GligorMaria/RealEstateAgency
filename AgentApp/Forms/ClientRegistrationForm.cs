...
using AgentApp.Core; // Client model now lives in Core

namespace ClientApp.Forms
{
    public class ClientRegistrationForm : Form
    {
        private TextBox txtUsername, txtPassword, txtFullName, txtEmail, txtPhone;
        private Button btnRegister;
        private string clientsFile = Path.Combine("Core", "Data", "Clients.json");
        ...
        {
            var clients = DataHandler.Load<Client>(clientsFile);

            if (Array.Exists(clients, c => c.Username == txtUsername.Text))
            {
            ...
            DataHandler.Add(clientsFile, newClient);
